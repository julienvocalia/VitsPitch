...
@dataclass
class VitsPitchAudioConfig(Coqpit):
    fft_size: int = 1024
...
class VitsPitchDataset(TTSDataset):
...
class VitsPitch(BaseTTS):
    ...
    @staticmethod
    def init_from_config(config: "VitsPitchConfig", samples: Union[List[List], List[Dict]] = None, verbose=True):
...
class VitsPitchCharacters(BaseCharacters):
    ...
    def init_from_config(config: Coqpit):
        if config.characters is not None:
            ...
            return (
                VitsPitchCharacters(graphemes=_letters, ipa_characters=_letters_ipa, punctuations=_punctuations, pad=_pad),
                config,
            )
        characters = VitsPitchCharacters()
